...
function imn_iframe_message_shortcode($atts) {
    ...
    <div class="iframe-container">
        <iframe id="imnIframe" src="<?php echo esc_url($a['src']); ?>" 
                width="<?php echo esc_attr($a['width']); ?>" height="<?php echo esc_attr($a['height']); ?>" 
    ...
    <script>
        window.addEventListener('message', function(event) {
            ...
            if (event.origin !== '<?php echo esc_url($a['src']); ?>') {
            ...
            if (message && message.type === 'navigate' && message.url) {
                window.location.href = message.url; // Change the parent window's URL
            }
        });
    </script>
